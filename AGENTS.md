...
- **共享站点组件**（`src/lib/components/site/`，优先复用，禁止在页面里重写）：`stat-tile`（统计瓦片）、`filter-pills`（ToggleGroup 筛选）、`collapsible-section`（后台折叠区）、`share-buttons`（分享）、`related-links`（相关链接卡）、`guide-hero`（指南缺图兜底）、`confirm-dialog`（危险操作确认，替代 window.confirm）、`image-upload`（客户端压缩 WebP → /api/media/upload，回填 URL 字段，所有 R2 图片字段必须附带）、`account-nav` / `admin-sidebar` / `supplier-sidebar`（三门户导航）、`mark`（品牌八角星徽记，唯一星形几何源，variants `outline|solid|pattern`；纹理落点只用 `.pattern-girih` 白名单：hero 容器/footer/sidebar 品牌条/登录注册/404，禁止列表网格与弹窗）、`certification-seal`（认证印章：圆环+缩写+状态色，supplier/product/verify 认证展示唯一源）、`icon`（品类图标，`verified` prop 叠加认证角标）、`branded-empty-media`（Empty 图标位唯一源：八角星水印 + lucide 图标居中，页面禁止直接用 `ui/empty` 的 `EmptyMedia`）。品牌资产（favicon/manifest 图标/og-default.png）一律 `pnpm brand` 生成到 `/static/brand`（非 R2），改 token 后重跑并同步 `app.html` theme-color hex。

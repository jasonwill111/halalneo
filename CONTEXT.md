...
| CMS Pages          | `/[slug]`                                                                                                                                           | ✅ Dynamic public page renderer — renders a `pages` table row (runtime lookup via `/api/pages/[slug]`; no prerendered entries)                                                                                                                                                                                                     |
| Export Docs        | `/export-docs`                                                                                                                                      | ✅ Trade document template library (static templates, client-side download)                                                                                                                                                                                                                                                        |
...
### API Endpoints (51 +server.ts files)
...
| `/api/success-stories/[slug]`   | GET, DELETE             | Story detail (GET cached public); DELETE admin-only                                                                                                                                                                                                                                                            |
...
| `/api/chat`                     | POST                    | AI chat — Vercel AI SDK `streamText` + `createAgnes()` (`agnes-3.0-flash`, `#lib/server/ai/agnes.ts`) + `HALAL_SYSTEM_PROMPT`, UIMessage stream response; 401 without session, 503 without `AGNES_API_KEY`                                                                                                     |
| `/rss.xml`                      | GET                     | Blog RSS feed (DB-driven, hourly `cachedQuery`)                                                                                                                                                                                                                                                                |
| `/robots.txt`                   | GET                     | Robots file — allows AI crawlers to read, disallows `/admin/` `/account/` `/supplier/` `/api/`, training opt-out                                                                                                                                                                                               |
...
| AI         | Vercel AI SDK 7.0.97（唯一 AI 实现，2026-09-20 起移除 Mastra）+ Agnes `agnes-3.0-flash` (AIHub, via `createAgnes()` in `#lib/server/ai/agnes.ts`; backend `streamText` + `toUIMessageStream()`, frontend `@ai-sdk/svelte` `Chat` + `DefaultChatTransport`, no raw `chat/completions` fetch) |
...
- Dynamic `/robots.txt` route (cached `max-age=86400`) — `Disallow: /admin/`, `/account/`, `/supplier/`, `/api/` for `User-agent: *`
- AI answer engines explicitly allowed to read public content (GEO): GPTBot, OAI-SearchBot, ChatGPT-User, ClaudeBot, Claude-Web, anthropic-ai, PerplexityBot, Google-Extended, Applebot-Extended, Bytespider
- Aggressive scraper CCBot fully disallowed; training opted out via Cloudflare AI content signal (`ai-train=no` header), not robots directives

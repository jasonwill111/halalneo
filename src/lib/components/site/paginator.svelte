...
{#if totalPages > 1}
	<nav class="mt-6 flex items-center justify-center gap-1" aria-label="Pagination">
		<Button
			variant="outline"
			size="icon"
			class="press-scale size-8 transition-transform duration-fast ease-spring"
			disabled={page === 1}
		...
		{#each Array(totalPages) as _, i (i + 1)}
			...
			{#if p === 1 || p === totalPages || Math.abs(p - page) <= 1}
				<Button
					...
					size="icon"
					class="press-scale size-8 text-xs transition-transform duration-fast ease-spring"
					aria-label={`Page ${p}`}

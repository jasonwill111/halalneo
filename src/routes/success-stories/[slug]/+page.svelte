...
<svelte:head>
	{@html `<script type="application/ld+json">${JSON.stringify({
		...
		author: story?.supplierSlug ? { '@type': 'Organization', name: data.supplierName ?? story.supplierSlug } : { '@type': 'Organization', name: 'HalalNeo' },
		datePublished: story?.createdAt ? new Date(story.createdAt).toISOString() : new Date().toISOString(),
		dateModified: story?.updatedAt ? new Date(story.updatedAt).toISOString() : new Date().toISOString(),
		publisher: {
			'@type': 'Organization',
			name: 'HalalNeo',
			url: 'https://halalneo.com',
			logo: { '@type': 'ImageObject', url: 'https://halalneo.com/api/media/og-default.png', width: 600, height: 600 }
		},
		mainEntityOfPage: {
			'@type': 'WebPage',
			'@id': `https://halalneo.com/success-stories/${data.slug}`
		},
		...(story.dealValue ? { about: [{ '@type': 'MonetaryAmount', currency: 'USD', value: story.dealValue }] } : {})
	})}</script>`}
</svelte:head>
              ...
              <CardContent class="space-y-3 p-5">
                ...
                <div class="space-y-3">
                  {#each data.related.slice(0, 5) as s (s.slug)}
                    ...
                    >
                      <h5 class="text-xs font-medium group-hover:text-primary line-clamp-2">{s.title}</h5>
                      <p class="mt-0.5 text-[10px] text-muted-foreground">{s.dealValue ?? s.buyerCountry}</p>
                    </a>
                  {/each}
                </div>
              </CardContent>

...
<svelte:head>
	...
	<meta name="description" content={data.item?.excerpt?.slice(0, 160) ?? data.item?.content?.slice(0, 160)} />
	{#if blogSchema}
...
<div class="container mx-auto max-w-3xl px-4 py-8">
	{#if data.item}
		<Breadcrumb items={[{ label: 'Blog', href: '/blog' }, { label: data.item.title ?? 'Blog Post' }]} />
		<article class="space-y-8">
			...
			<Card>
				<CardContent class="flex items-center justify-between p-4">
					...
					<div class="flex gap-2">
						...
						<Button variant="outline" size="sm" href="https://twitter.com/intent/tweet?text={encodeURIComponent(data.item.title ?? '')}&url={encodeURIComponent('https://halalneo.com/blog/' + data.slug)}" target="_blank" rel="noopener">Twitter</Button>
					</div>
				</CardContent>
			</Card>
				...
				<section class="space-y-4 border-t border-border pt-8">
					...
					<div class="grid gap-3 sm:grid-cols-3">
						{#each data.related as related}
							<a href={localizeHref(`/blog/${related.slug}`)} class="group block rounded-xl border border-border bg-card p-4 transition-all hover:shadow-md">
								<div class="flex items-center gap-2 text-xs text-muted-foreground mb-2">
									<span class="font-medium text-foreground/80">{related.author.name}</span>
									<span>·</span>
									<time>{related.date}</time>
								</div>
								<h3 class="text-sm font-semibold leading-snug group-hover:text-primary transition-colors mb-2">{related.title}</h3>
								...
								{#if related.tags?.length}
									<div class="flex flex-wrap gap-1.5 mt-3">
										{#each related.tags as tag}
											<span class="inline-flex items-center rounded bg-secondary px-1.5 py-0.5 text-[10px] font-medium text-secondary-foreground">{tag}</span>
										{/each}
									</div>
								{/if}
							</a>
			...
			<h2 class="text-xl font-semibold tracking-tight mb-4">Explore Related Resources</h2>
			<div class="grid gap-4 sm:grid-cols-3">
				<a href={localizeHref('/products')} class="group block rounded-xl border border-border bg-card p-5 transition-all hover:shadow-md">
					<div class="mb-2">
					...
					<h3 class="text-sm font-semibold group-hover:text-primary transition-colors">Browse Halal Products</h3>
					<p class="text-xs text-muted-foreground mt-1">Explore our catalog of certified halal products.</p>
					<span class="text-xs font-medium text-primary mt-2 inline-block group-hover:translate-x-0.5 transition-transform">Browse Products &rarr;</span>
				</a>
				<a href={localizeHref('/suppliers')} class="group block rounded-xl border border-border bg-card p-5 transition-all hover:shadow-md">
					<div class="mb-2">
					...
					<h3 class="text-sm font-semibold group-hover:text-primary transition-colors">Find Certified Suppliers</h3>
					<p class="text-xs text-muted-foreground mt-1">Connect with verified halal-certified suppliers.</p>
					<span class="text-xs font-medium text-primary mt-2 inline-block group-hover:translate-x-0.5 transition-transform">Find Suppliers &rarr;</span>
				</a>
				<a href={localizeHref('/knowledge-base')} class="group block rounded-xl border border-border bg-card p-5 transition-all hover:shadow-md">
					<div class="mb-2">
						<svg class="size-5 text-primary/60 group-hover:text-primary transition-colors" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M12 6.042A8.967 8.967 0 006 3.75c-1.052 0-2.062.18-3 .512v14.25A8.987 8.987 0 016 18c2.305 0 4.408.867 6 2.292m0-14.25a8.966 8.966 0 016-2.292c1.052 0 2.062.18 3 .512v14.25A8.987 8.987 0 0018 18a8.967 8.967 0 00-6 2.292m0-14.25v14.25"/></svg>
					</div>
					<h3 class="text-sm font-semibold group-hover:text-primary transition-colors">Read Knowledge Base</h3>
					<p class="text-xs text-muted-foreground mt-1">Guides, certifications, and halal compliance insights.</p>
					<span class="text-xs font-medium text-primary mt-2 inline-block group-hover:translate-x-0.5 transition-transform">Read Articles &rarr;</span>
				</a>
			</div>
		</div>
	</article>

<script lang="ts">
	...
	const benefits = [
		{
			...
			desc: 'Connect with certified buyers across ASEAN, Gulf and Europe.',
			color: 'bg-info/10 text-info'
		},
		{
			icon: ShieldCheck,
			title: 'Certification trust',
			desc: 'Showcase verified halal certificates buyers can check.',
			color: 'bg-success/10 text-success'
		},
		{
			icon: BarChart3,
			title: 'Sales analytics',
			desc: 'Track inquiries, product views and market demand.',
			color: 'bg-warn/10 text-warn'
		},
		{
			icon: Users,
			title: 'Direct inquiries',
			desc: 'Receive qualified RFQs directly in your dashboard.',
	...
	const canContinue = $derived(
		step === 1
			? company.trim() !== '' &&
					country.trim() !== '' &&
					businessTypes.some((t) => t.value === bizType)
			: true
	);

	const businessTypeError = $derived(
		step === 1 && bizType && !businessTypes.some((t) => t.value === bizType)
			? 'Choose manufacturer, wholesaler, or trader.'
			: ''
	);
	...
	const step1Schema = z.object({
		company: z
			.string()
			.trim()
			.min(2, 'Please enter your company legal name.')
		...
		bizType: z
			...
			.refine(
				(v) => businessTypes.some((t) => t.value === v),
				'Choose manufacturer, wholesaler, or trader.'
			),
	...
	async function next() {
		...
		const issues = [
			...(step1Schema
				.safeParse({ company, country, bizType, products, email })
				.error?.issues.map((i) => ({ key: String(i.path[0] ?? ''), message: i.message })) ?? []),
			...(step2Schema
				.safeParse({ cert, regNo, website })
				.error?.issues.map((i) => ({ key: String(i.path[0] ?? ''), message: i.message })) ?? [])
		];
		if (issues.length > 0) {
			applyIssues(issues);
		...
		try {
			...
			step = 4;
			toast.success(
				data?.message ?? 'Application submitted — our team will review it within 1–3 business days.'
			);
		} catch {
			submitError =
...
<svelte:head>
	...
	<meta
		name="description"
		content="Apply to list your halal-certified products on HalalNeo and reach global buyers."
	/>
</svelte:head>
...
<section class="mx-auto max-w-6xl">
	...
	<section class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-2">
		{#each benefits as b (b.title)}
			<Card class="p-3">
				<CardContent class="space-y-1 p-0">
					...
					<h3 class="text-center text-2xs-plus leading-tight font-semibold">{b.title}</h3>
				...
				{#each steps as s, i (s)}
					...
					>
						<div class="flex flex-col items-center">
							<div
								class="flex size-7 items-center justify-center rounded-full text-2xs font-semibold {step >
								i
							...
							<span class="mt-1 text-2xs {step > i ? 'text-foreground' : 'text-muted-foreground'}"
								>Step {i + 1}: {s}</span
							>
						</div>
						{#if i < steps.length - 1}
				...
				{#if step === 1}
					<div class="space-y-2">
						<div class="space-y-0.5">
							<Label class="mb-1 block text-2xs"
								>Company Legal Name <span class="text-destructive">*</span></Label
							>
							<Input
								type="text"
								class="w-full"
								...
								oninput={() => {
									if (fieldErrors.company) fieldErrors = { ...fieldErrors, company: '' };
								}}
							/>
							{#if fieldErrors.company}<FieldError class="text-2xs"
									>{fieldErrors.company}</FieldError
								>{/if}
						</div>
						<div class="grid grid-cols-1 gap-2 sm:grid-cols-2">
							<div class="space-y-0.5">
								<Label class="mb-1 block text-2xs"
									>Country <span class="text-destructive">*</span></Label
								>
								<Input
									type="text"
									placeholder="Country"
								...
								/>
								{#if fieldErrors.country}<FieldError class="text-2xs"
										>{fieldErrors.country}</FieldError
									>{/if}
							</div>
							<div class="space-y-0.5">
								<Label class="mb-1 block text-2xs"
									>Business Type <span class="text-destructive">*</span></Label
								>
								<Select type="single" bind:value={bizType}>
									<SelectTrigger
										class="w-full"
										aria-invalid={fieldErrors.bizType ? true : undefined}
									>
						...
						<div class="space-y-0.5">
							...
							<Input
								...
								oninput={() => {
									if (fieldErrors.products) fieldErrors = { ...fieldErrors, products: '' };
								}}
							/>
							{#if fieldErrors.products}<FieldError class="text-2xs"
									>{fieldErrors.products}</FieldError
								>{/if}
						</div>
						<div class="space-y-0.5">
							<Label class="mb-1 block text-2xs"
								>Contact Email <span class="text-destructive">*</span></Label
							>
							<Input
								type="email"
							...
							/>
							{#if fieldErrors.email}<FieldError class="text-2xs">{fieldErrors.email}</FieldError
								>{/if}
						</div>
					</div>
				{:else if step === 2}
					<div class="space-y-2">
						<div class="space-y-0.5">
							...
							<Input
								...
								oninput={() => {
									if (fieldErrors.cert) fieldErrors = { ...fieldErrors, cert: '' };
								}}
							/>
							{#if fieldErrors.cert}<FieldError class="text-2xs">{fieldErrors.cert}</FieldError
								>{/if}
						</div>
						<div class="space-y-0.5">
							...
							<Input
								...
								oninput={() => {
									if (fieldErrors.regNo) fieldErrors = { ...fieldErrors, regNo: '' };
								}}
							/>
							{#if fieldErrors.regNo}<FieldError class="text-2xs">{fieldErrors.regNo}</FieldError
								>{/if}
						</div>
						<div class="space-y-0.5">
							...
							/>
							{#if fieldErrors.website}<FieldError class="text-2xs"
									>{fieldErrors.website}</FieldError
								>{/if}
						</div>
					</div>
				{:else if step === 3}
					<div class="space-y-2">
						...
						<dl class="space-y-1 rounded-lg bg-muted/50 p-3 text-2xs-plus">
							...
							<div class="flex justify-between gap-2">
								...
								<dd class="font-medium">{country || '—'}</dd>
							</div>
							<div class="flex justify-between gap-2">
								<dt class="text-muted-foreground">Type</dt>
								<dd class="font-medium">{bizType || '—'}</dd>
							</div>
							<div class="flex justify-between gap-2">
								<dt class="text-muted-foreground">Products</dt>
								<dd class="font-medium">{products || '—'}</dd>
							</div>
							<div class="flex justify-between gap-2">
							...
							<div class="flex justify-between gap-2">
								<dt class="text-muted-foreground">Certification</dt>
								<dd class="font-medium">{cert || '—'}</dd>
							</div>
						</dl>
					</div>
				{:else}
					<div class="rounded-lg bg-primary/5 p-4 text-center">
						...
						<p class="mt-1 text-2xs text-muted-foreground">
							Our team will review your details and respond within 1–3 business days. We will email
							you at the address you provided.
						</p>
						<div class="mt-3 flex flex-col items-center gap-1 text-2xs text-muted-foreground">
							<a href="/pricing" class="underline underline-offset-2 hover:text-foreground"
								>View pricing details</a
							>
							<a href="/faq" class="underline underline-offset-2 hover:text-foreground"
								>Read the supplier FAQ</a
							>
						</div>
					</div>
				{/if}

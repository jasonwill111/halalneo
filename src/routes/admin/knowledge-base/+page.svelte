<script lang="ts">
	...
	async function loadArticles() {
		...
		loadError = null;
		try {
			// status=all â†?admin overview (drafts included), never edge-cached.
	...
	async function confirmedDelete() {
		...
		try {
			...
			toast.error('Network error â€?the article was not deleted.');
		} finally {
			deleting = false;
			confirmSlug = null;
			confirmTitle = '';
...
<svelte:head>
	<title>Knowledge Base â€?HalalNeo Admin</title>
...
<div class="space-y-4 sm:space-y-6">
	...
	{#if loadError}
		<!-- Real failure: a retry action, never the "empty" copy (Â§3.1). -->
		<div
								...
								{:else}
									<Empty>
										...
										<div class="space-y-1">
											...
											<p class="text-sm text-muted-foreground">
												Nothing matches â€œ{search}â€? Try another word.
											</p>
										</div>
										<EmptyContent>

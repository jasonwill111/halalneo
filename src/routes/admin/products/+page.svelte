	...
	<DialogContent class="max-h-[85dvh] overflow-y-auto sm:max-w-lg">
		...
		<div class="flex flex-col gap-4">
			...
			<button
				type="button"
				class="flex items-center gap-2 rounded-md px-1 py-1.5 text-sm font-medium select-none hover:bg-muted"
				onclick={() => (pricingExpanded = !pricingExpanded)}
			>
				{#if pricingExpanded}
					<ChevronDown class="size-4" />
				{:else}
				...
				<div class="flex flex-col gap-4 pl-6">
					...
					<div class="grid grid-cols-2 gap-4">
						<Field.Field>
							...
							<Input bind:value={form.priceMin} type="number" min="0" step="0.01" placeholder="0.00" />
						</Field.Field>
						<Field.Field>
							<Field.FieldLabel>Price Max</Field.FieldLabel>
							<Input bind:value={form.priceMax} type="number" min="0" step="0.01" placeholder="0.00" />
						</Field.Field>
					</div>
					...
					<Field.Field>
						...
						{#if form.features.trim()}
							<div class="flex flex-wrap gap-1 pt-1">
								{#each form.features.split(',').map((f) => f.trim()).filter(Boolean) as tag (tag)}
					...
					<div class="flex flex-col gap-2">
						...
						{#each form.faqs as faq, i (faq.id)}
							<div class="flex flex-col gap-2 rounded-md border p-3">
								...
								<Textarea
									value={faq.answer}
									oninput={(e) => updateFaq(i, 'answer', (e.currentTarget as HTMLTextAreaElement).value)}
									placeholder="Answer"
									rows={2}
								/>
							</div>
			...
			{#if mediaExpanded}
				<div class="flex flex-col gap-4 pl-6">
					...
					<Field.Field>
						...
						<Field.FieldDescription>Comma-separated list of additional image URLs.</Field.FieldDescription>
					</Field.Field>
			...
			{#if seoExpanded}
				<div class="flex flex-col gap-4 pl-6">
					...
					<Field.Field>
						...
						<Input bind:value={form.keywords} placeholder="Comma separated: halal, product, certification" />
					</Field.Field>
				</div>
			{/if}

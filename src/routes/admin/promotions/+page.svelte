<script lang="ts">
	...
	import type { ApiList, PromotionItem } from '#lib/types/api.js';
	...
	let selected = $state<PromotionItem | null>(null);
	...
	async function loadItems() {
		...
		try {
			...
			if (res.ok) {
				const data = (await res.json()) as ApiList<PromotionItem>;
				items = data.items ?? [];
				total = data.total ?? 0;
			}
		} finally {
	...
	function viewDetail(item: PromotionItem) {
		selected = item;
		detailOpen = true;
	}

	function confirmDelete(item: PromotionItem) {
		confirmId = item.id;
				...
				{:else if items.length === 0}
					<TableRow>
						<TableCell colspan={7} class="py-8">
							<Empty>
								<BrandedEmptyMedia
									><PackageIcon class="size-6 text-muted-foreground" /></BrandedEmptyMedia
								>
								<div class="space-y-1">
									<p class="font-medium">No promotions found</p>
									<p class="text-sm text-muted-foreground">
										No deals or special offers to display.
									</p>
								</div>
							</Empty>
						</TableCell>
					...
					{#each items as p (p.id)}
						<TableRow>
							...
							<TableCell class="text-xs">
								{p.priceMin ?? '—'}{p.priceMax ? ` – ${p.priceMax}` : ''}{p.priceUnit
									? ` ${p.priceUnit}`
									: ''}
							</TableCell>
							...
							<TableCell>
								<Badge
									variant="secondary"
									class={`px-1.5 py-0.5 text-2xs capitalize ${statusColor(p.status ?? 'active')}`}
								>
									{p.status ?? 'active'}
								</Badge>
							</TableCell>

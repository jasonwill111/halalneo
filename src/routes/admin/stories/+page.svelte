...
<div class="flex items-center justify-between gap-2">
	<div>
		...
		<p class="text-sm text-muted-foreground">
			Editorial case studies — site page + supplier sections.
		</p>
	</div>
			...
			<Table>
				<TableHeader>
					<TableRow>
						...
						<TableHead class="text-end text-2xs">Actions</TableHead>
					</TableRow>
				</TableHeader>
				<TableBody>
					{#each stories as s (s.slug)}
						<TableRow>
							<TableCell class="max-w-[220px] truncate text-2xs-plus font-medium"
								>{s.title}</TableCell
							>
		...
		>
			<div class="grid gap-2 sm:grid-cols-2">
				<Field>
					...
					<Input
						type="text"
						bind:value={slug}
						placeholder="nusantara-foods-uae-deal"
						class="text-xs"
				...
				<Field>
					...
					<Input
						type="text"
						bind:value={dealValue}
						placeholder="$120K first order"
						class="text-xs"
					/>
				</Field>
			</div>
			...
			<Field>
				...
				<Textarea
					...
					rows={6}
					class="text-xs"
					placeholder="The story: buyer need, how the match happened, outcome..."

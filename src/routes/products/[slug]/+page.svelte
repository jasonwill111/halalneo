<script lang="ts">
	...
	const breadcrumbs = $derived.by(() => {
		const items = [{ name: 'Home', url: baseUrl }, { name: 'Products', url: `${baseUrl}/products` }];
	...
	const faqs = $derived(
		Array.isArray(item?.faqs) ? item.faqs : []
	);
...
{#if item}
	<div class="mx-auto max-w-5xl space-y-6">
		<Breadcrumb items={[{ label: 'Products', href: '/products' }, { label: item.name ?? 'Product' }]} />

		<!-- Top: Image + Info -->
		<div class="grid gap-6 lg:grid-cols-5">
			<!-- Image -->
			<div class="lg:col-span-2">
				<div class="aspect-square w-full rounded-xl border border-border bg-muted flex items-center justify-center">
					{#if item.image}
						<img src={item.image} alt={item.name} class="h-full w-full rounded-xl object-cover" loading="lazy" decoding="async" width="512" height="512" onerror={(e) => { e.currentTarget.style.display='none'; }} />
					{:else}
			...
			<div class="space-y-4 lg:col-span-3">
				...
				<div class="flex flex-wrap gap-1.5">
					{#if item.certStatus === 'certified'}
						<Badge variant="secondary" class="gap-1">
							<ShieldCheck class="size-2.5 text-green-600"></ShieldCheck>
							Halal Certified
						</Badge>
					{/if}
				...
				<!-- Price card -->
				<div class="rounded-xl bg-card shadow-sm p-4 shadow-sm space-y-3">
					<div>
				...
				<div class="flex gap-2">
					<Button class="flex-1 gap-2" onclick={() => { inquiryResult = null; inquiryOpen = true; }}>
						<Send class="size-4"></Send>
						Send Inquiry
					</Button>
					<Button variant="outline" size="icon" class="h-12 w-12 shrink-0" onclick={handleToggleFavorite} aria-label={favorited ? 'Remove from favorites' : 'Add to favorites'}>
				...
				<div class="flex items-center gap-2">
					...
					<a href="https://www.facebook.com/sharer/sharer.php?u={encodeURIComponent(page.url.href)}" target="_blank" rel="noopener noreferrer">
						<Button variant="outline" size="icon" class="size-8 rounded-full text-[10px]">f</Button>
					</a>
					<a href="https://www.linkedin.com/sharing/share-offsite/?url={encodeURIComponent(page.url.href)}" target="_blank" rel="noopener noreferrer">
						<Button variant="outline" size="icon" class="size-8 rounded-full text-[10px]">in</Button>
					</a>
					<a href="https://twitter.com/intent/tweet?url={encodeURIComponent(page.url.href)}" target="_blank" rel="noopener noreferrer">
		...
		<div class="flex gap-4 border-b border-border overflow-x-auto">
			{#each ['description', 'specs', 'certifications', 'faq', 'resources'] as tab}
				<Button
					...
					class={`border-b-2 rounded-none pb-3 text-sm font-medium whitespace-nowrap ${activeTab === tab ? 'border-primary text-foreground' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
				>
		...
		<div class="flex flex-col gap-6 lg:grid lg:grid-cols-3 lg:gap-8">
			<div class="lg:col-span-2 space-y-4">
				{#if activeTab === 'description'}
					...
					<div class="prose prose-sm max-w-none text-sm text-muted-foreground leading-relaxed">
					{#if item.description}
					...
					{#if features.length > 0}
						<h3 class="text-sm font-medium text-foreground mt-4">Key Features</h3>
						<ul class="text-sm space-y-1 mt-1 list-disc list-inside">
							{#each features as feature}
								<li>{typeof feature === 'string' ? feature : feature.value ?? JSON.stringify(feature)}</li>
							{/each}
						</ul>
					{/if}
						...
						<dl class="space-y-2 text-sm">
							{#each Object.entries(specifications) as [key, value]}
								<div class="flex justify-between border-t border-border pt-2">
									...
									<dd class="font-medium">{typeof value === 'string' ? value : JSON.stringify(value)}</dd>
								</div>
							{/each}
						</dl>
					{:else}
				...
				{:else if activeTab === 'certifications'}
					...
					<div class="space-y-2">
						{#if item.certStatus === 'certified'}
							<div class="flex items-center gap-2 rounded-lg bg-green-500/5 p-3">
								<ShieldCheck class="size-5 text-green-600"></ShieldCheck>
								<div>
									<p class="text-sm font-medium">Halal Certified</p>
									<p class="text-xs text-muted-foreground">Verified by recognized certifying body</p>
								</div>
							</div>
						{:else}
				...
				{:else if activeTab === 'faq'}
					...
					{#if faqs.length > 0}
						<div class="space-y-3">
							{#each faqs as faq}
								<Card class="p-3">
									<h3 class="text-sm font-medium">{typeof faq === 'string' ? faq : faq.question ?? faq.q ?? 'Question'}</h3>
									<p class="mt-1 text-xs text-muted-foreground">{typeof faq === 'string' ? '' : faq.answer ?? faq.a ?? ''}</p>
								</Card>
							{/each}
						</div>
					{:else}
						...
						<div class="space-y-2">
							{#each resources as res}
								<div class="flex items-center justify-between rounded-lg border border-border p-3">
									<span class="text-sm font-medium">{typeof res === 'string' ? res : res.name ?? 'Resource'}</span>
			...
			<div class="space-y-4">
				<Card class="p-4">
					...
					<dl class="space-y-2 text-sm">
						<div class="flex justify-between"><dt class="text-muted-foreground">Origin</dt><dd class="font-medium">{item.originCountry ?? '—'}</dd></div>
						<div class="flex justify-between border-t border-border pt-2"><dt class="text-muted-foreground">Category</dt><dd class="font-medium">{item.categorySlug}</dd></div>
						<div class="flex justify-between border-t border-border pt-2"><dt class="text-muted-foreground">MOQ</dt><dd class="font-medium">{item.moq ?? '—'}</dd></div>
						<div class="flex justify-between border-t border-border pt-2"><dt class="text-muted-foreground">Units</dt><dd class="font-medium">{item.units ?? '—'}</dd></div>
						<div class="flex justify-between border-t border-border pt-2"><dt class="text-muted-foreground">Price</dt><dd class="font-medium">{priceDisplay}</dd></div>
					</dl>
				</Card>
				...
				<Card class="p-4">
					...
					<Button href={localizeHref(`/suppliers/${item.supplierSlug}`)} variant="outline" size="sm" class="mt-2 w-full">

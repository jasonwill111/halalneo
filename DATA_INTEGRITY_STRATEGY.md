...
<div class="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 lg:grid-cols-4">
	{#each products as product}
		<article class="card">
			<!-- Mobile: Minimal info -->
			<h3 class="truncate text-sm">{product.name}</h3>
			<p class="text-xs text-gray-600">{product.price}</p>

			<!-- Tablet+ show more (hidden sm:block) -->
			<p class="hidden text-sm sm:block">{product.shortDescription}</p>
		</article>
	{/each}
</div>
```
...
const orchestrator = new QualityGateOrchestrator();
const result = await orchestrator.evaluate({ name: 'Product', priceMin: 100 });
console.log(result);
```
...
_Author: HalalNeo Development Team_
_Version: 1.0.0_

...
check(
	'mobile tab bar uses rounded-xl',
	!/glass-strong[^"]*rounded-2xl/.test(home),
	'popover + dock'
);
...
check(
	'desktop Trade left of Resources',
	tradeIdx !== -1 && resIdx !== -1 && tradeIdx < resIdx,
	'header dropdown order'
);

// Screenshot for visual confirmation
await page.goto(`${BASE}/`, { waitUntil: 'networkidle' });
await page.screenshot({
	path: 'D:\\Dev Projects\\halalneo\\.scratch\\ui-smoke\\radius-home.png',
	fullPage: true
});
const mob = await browser.newPage({ viewport: { width: 390, height: 844 } });
await mob.goto(`${BASE}/`, { waitUntil: 'networkidle' });
await mob.screenshot({
	path: 'D:\\Dev Projects\\halalneo\\.scratch\\ui-smoke\\radius-home-mobile.png',
	fullPage: true
});
...
check(
	'mobile Explore Trade first',
	popText.indexOf('TRADE') !== -1 && popText.indexOf('TRADE') < popText.indexOf('RESOURCES'),
	'group order'
);

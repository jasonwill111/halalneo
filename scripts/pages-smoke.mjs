...
check(
	...
	text.includes('All products') && text.includes('listing'),
	'grid section'
);
...
// filter interaction
await page
	.getByRole('button', { name: /Beverages/i })
	.first()
	.click();
...
check(
	'detail: supplier card named',
	text.includes('Verified halal supplier'),
...
text = (await page.textContent('body')) ?? '';
check(
	'suppliers: type filter cards',
	text.includes('Manufacturer') && text.includes('Wholesaler') && text.includes('Trader'),
	'3 type cards'
);
...
await page
	.getByRole('button', { name: /Manufacturer \d/ })
	.first()
	.click();
await page.waitForTimeout(600);
text = (await page.textContent('body')) ?? '';

...
check(
	...
	pdText.includes('HalalNeo') && (pdText.includes('Halal Certified') || pdText.includes('product')),
	'content'
);
...
check(
	'old /suppliers/x 301',
	r1s.status() === 301 && (r1s.headers().location ?? '').includes(`/supplier/`),
	r1s.headers().location ?? ''
);
const r2s = await page.request.get(`${BASE}/products/${prodSlug}`, { maxRedirects: 0 });
check(
	'old /products/x 301',
	r2s.status() === 301 && (r2s.headers().location ?? '').includes(`/product/`),
	r2s.headers().location ?? ''
);
const r3s = await page.request.get(`${BASE}/categories/${catSlug}`, { maxRedirects: 0 });
check(
	'old /categories/x 301',
	r3s.status() === 301 && (r3s.headers().location ?? '').includes(`/category/`),
	r3s.headers().location ?? ''
);
...
check(
	'portal not shadowed by [slug]',
	obText.includes('Become a HalalNeo supplier'),
	'onboarding form'
);
...
check(
	'sitemap no old plural detail',
	!sm.includes('/products/') && !sm.includes('/suppliers/') && !sm.includes('/categories/'),
	'clean'
);

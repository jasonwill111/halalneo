...
for (const c of categories) {
	stmts.push(
		`INSERT INTO "categories" ("slug","name","description","parent_slug","icon","sort_order","created_at","updated_at") VALUES (${esc(c.slug)},${esc(c.name)},${esc(c.description ?? null)},${esc(c.parentSlug ?? null)},${esc(c.icon ?? null)},${c.sortOrder ?? 0},${now()},${now()});`
	);
}
...
for (const cb of certifyingBodies) {
	stmts.push(
		`INSERT INTO "certifying_bodies" ("id","name","country","standard","website","description","status","created_at","updated_at") VALUES (${esc(cb.id)},${esc(cb.name)},${esc(cb.country)},${esc(cb.standard ?? null)},NULL,NULL,'active',${now()},${now()});`
	);
}
...
for (const a of kbArticles) {
	stmts.push(
		`INSERT INTO "knowledge_base" ("slug","section","title","summary","body","tags","author","status","views","created_at","updated_at") VALUES (${esc(a.slug)},${esc(a.section)},${esc(a.title)},${esc(a.summary ?? null)},${esc(a.body ?? null)},${esc(JSON.stringify(a.tags))},${esc(a.author ?? null)},${esc(a.status ?? 'published')},${esc(a.views ?? 0)},${now()},${now()});`
	);
}

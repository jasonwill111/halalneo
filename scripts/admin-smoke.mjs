...
check(
	...
	(await page.textContent('body'))?.includes('Invalid') ?? page.url().includes('/admin/login'),
	'stays on login'
);
...
check(
	'widget shows pending row',
	adminText.includes('PureHarvest Snacks') || adminText.includes('pending'),
	'row content'
);

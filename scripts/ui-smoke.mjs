...
check(
	'pricing Apply CTAs',
	(await page.getByRole('link', { name: 'Apply' }).count()) >= 3,
	'cta count'
);
check(
	'pricing FAQ rewritten',
	bodyText.includes('Will I be charged during supplier test mode?'),
	'faq q1'
);
...
check(
	'step3 review shown',
	(await page.textContent('body'))?.includes('Review your application') ?? false,
	'review panel'
);
...
check(
	'submit success',
	(await page.textContent('body'))?.includes('Application submitted') ?? false,
	'success panel'
);
...
check(
	'pricing mobile renders',
	(await mob.textContent('body'))?.includes('Supplier test mode') ?? false,
	'banner on mobile'
);
...
check(
	'onboarding mobile renders',
	(await mob.textContent('body'))?.includes('Become a HalalNeo supplier') ?? false,
	'h1 on mobile'
);

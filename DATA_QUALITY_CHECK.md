...
export async function checkDataIntegrity(): Promise<CheckResult[]> {
  ...
  const invalidProducts = await db.select().from(products)
    .where(
      and(
  ...
  const invalidSuppliers = await db.select().from(suppliers)
    .where(
      and(
  ...
  const invalidCategories = await db.select().from(categories)
    .where(
      or(
        eq(categories.name, ''),
  ...
  const invalidGuides = await db.select().from(marketGuides)
    .where(
      or(
...
function checkProductQuality(product: any, issues: Issue[]): QualityScore {
  ...
  // 必填字段
  if (!product.name) {
    issues.push({ type: 'missing-field', field: 'name', value: null, expected: 'string (required)' });
    score -= 20;
  }
  
  if (!product.priceMin || product.priceMin <= 0) {
    issues.push({ type: 'invalid-format', field: 'priceMin', value: product.priceMin, expected: 'positive number' });
    score -= 15;
  }
...
function generateRecommendations(issues: Issue[]): string[] {
  ...
  issues.forEach(issue => {
    switch (issue.type) {
      case 'missing-field':
        recs.push(`Complete the "${issue.field}" field in ${issue.value ? 'this product' : 'all products'}`);
        break;
      case 'invalid-format':
...
async function checkRelations() {
  ...
  const orphanProducts = await db.select().from(products)
  ...
  const suppliersMissingBodies = await db.select().from(suppliers)
...
<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
  <Card>
    <CardContent class="p-6">
      ...
      <div class="text-2xl font-bold {qualityColor(qualityStats.productAverages)}">
...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell class={qualityColor(stat.score)}>
        {stat.score}%
      </TableCell>
      <TableCell>
        <Badge variant={stat.issues > 10 ? 'destructive' : 'secondary'}>
...
<Table>
  ...
  <TableBody>
    ...
    <TableRow>
      ...
      <TableCell>
        <Badge variant={item.qualityScore >= 80 ? 'success' : 'destructive'}>
          {item.qualityScore}%
        </Badge>
      </TableCell>
      <TableCell>
        {#if item.qualityScore < 80}
          <Button size="sm" variant="outline" onclick={() => navigateTo(`/admin/products/${item.slug}/edit`)}>
            Fix Issues
          </Button>
        {/if}
      </TableCell>
